...
在本课程里, 一个实用解法是 fresh `messages[]` 隔离: 以 `messages=[]` 启动一个子智能体。子智能体进行探索、读取文件、运行命令。完成后, 只有最终的文本响应返回给父智能体。子智能体的全部消息历史被丢弃。意思是整个任务交给主智能体来做，在做任务的中间过程里会产生很多内容，这些内容会占据上下文太多空间，解决方法是，把这个任务交给子智能体，智能体只把任务的结果给到主智能体，那么只会有任务的结果去占据上下文，腾出来了很多空间
